...
int main (int argc, char *argv[])
{
    ...
    if (config_file_name) config_file_name = g_strdup(config_file_name);
		 ...
		 * old location.
		 */
		config_file_name = g_strdup_printf("%s/%s/%s", g_get_user_config_dir(), "xpde", CONFIG_FILE_NAME);
		if (!g_file_test(config_file_name, G_FILE_TEST_IS_REGULAR)) {
			/* create directory for new config file location */
			char *path = g_strdup_printf("%s/%s", g_get_user_config_dir(), "xpde");
...
"xpde", config_file_name_old, config_file_name, "xpde", config_file_name_old);
